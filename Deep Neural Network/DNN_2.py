...
class neural_network_2():
  ...
  def __init__(self, layer_sizes, new_weights, new_biases):
    self.number_of_layers = len(layer_sizes)
    self.layer_sizes = layer_sizes

    self.weights_initializer(new_weights)
    self.biases_initializer(new_biases)
...
#%%%%%%%%%%%%%%%%%%%%%%%   Bias initialization from the 2nd layer  %%%%%%%%%%%%
  ...
  def biases_initializer(self, new_biases):
...
# Order of indices for w(j,k). Example : k = 1st layer and j = 2nd layers
  ...
  def weights_initializer(self, new_weights):
